...
use syn::{braced, parse::{discouraged::Speculative as _, Parse, ParseStream}, punctuated::Punctuated, spanned::Spanned as _, token::Brace, Block, Expr, Ident, Token, Type};
...
impl Parse for If {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        Ok(Self { if_token, t, is_token, condition, block, else_ifs, else_stmnt })
    }
}
...
impl Parse for Else {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        Ok(Self { else_token: input.parse()?, block: input.parse()? })
    }
}
impl ToTokens for If {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        self.if_token.to_tokens(tokens);
        self.condition.to_tokens(&self.t, self.is_token.0.span(), tokens);
        self.block.to_tokens(tokens);

        for else_if in &self.else_ifs {
            else_if.else_token.to_tokens(tokens);
            else_if.if_token.to_tokens(tokens);
            else_if.condition.to_tokens(&else_if.t, else_if.is_token.0.span(), tokens);
...
impl Parse for Match {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        Ok(Self { match_token, t, braces, arms, default_case_arm })
    }
}
...
impl Parse for MatchBody {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        if input.peek(Brace) {
            Ok(Self::Block { block: input.parse()?, comma: input.parse()? })
        } else {
            Ok(Self::Expr { expr: input.parse()?, comma: input.parse()? })
        }
    }
}
impl MatchBody {
...
impl Condition {
    ...
    fn to_tokens(&self, generic_t: &Type, eq_span: Span, tokens: &mut TokenStream) {
        ...
        for pair in self.0.pairs() {
            ...
            let or = pair.punct() // Will output no tokens when None (aka at end)
                .map(|bitor| {
                    let mut first = Punct::new('|', proc_macro2::Spacing::Joint);
                    let mut second = Punct::new('|', proc_macro2::Spacing::Alone);
